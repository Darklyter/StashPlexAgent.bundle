...
# Get your Plex token, and hostname for Plex. Port usually doesn't need changing.
# NB: this plugin connects via HTTPS, but ignores cert errors.
plex_token = "YOUR_PLEX_TOKEN_HERE"
plex_host = "localhost"
plex_port = "32400"

# Create tags for empty stashid (important!), as well as the other stashboxes, and add here as *integer*.
# If you don't use one of the listed stash-boxes, create some dummy tag ID regardless...
#   I don't check if tag exists or not, so this plugin will infinite loop if you missing some tag ID.
empty_stashid = 3737
stashdb = 3383
pmvstash = 3461
porndb = 3278
...
def processScene(scene):
    ...
    if not scene['organized']: # don't process scenes that aren't marked organized.
    ...
    if scene["urls"]: # If scene contains Plex URL, refresh Plex metadata
        for urls in reversed(scene["urls"]): # todo: remove reversed(); this was a quickfix for when a scene somehow got two plex urls - the most recent url is most likely the most "valid"
            if ("/library/metadata/" in urls):
                plexurl = urls.replace("plex", "https://" + plex_host + ":" + plex_port)
                plexurl += "/refresh?X-Plex-Token=" + plex_token
...
#        log.info(f"We run with {json_input}") # Uncomment for debugging what's sent to Stash
        ...
        request_tags = []
        stashbox_tags = []
        stashbox_tags.append(frozenset({stashdb, pmvstash, porndb, fansdb}))
